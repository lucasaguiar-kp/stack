import { APP_ERROR_MAP } from "../../../core/errors/error-map";
import { handleError } from "../../../core/http/error-handler";
import { protectedProcedure } from "../../../procedures";
import { updateDeviceGroupSchema } from "./schema";
import { updateDeviceGroup } from "./service";

export const updateDeviceGroupRoute = protectedProcedure
  .route({
    method: "PATCH",
    path: "/groups/:groupId",
    description: "Update a group",
    tags: ["groups"],
    successStatus: 200,
  })
  .meta({ authRequired: true })
  .input(updateDeviceGroupSchema.omit({ requesterId: true }))
  .errors({
    UNAUTHORIZED: APP_ERROR_MAP.UNAUTHORIZED,
    USER_NOT_FOUND: APP_ERROR_MAP.USER_NOT_FOUND,
    DEVICE_GROUP_NOT_FOUND: APP_ERROR_MAP.DEVICE_GROUP_NOT_FOUND,
    DEVICE_GROUP_NAME_ALREADY_EXISTS: APP_ERROR_MAP.DEVICE_GROUP_NAME_ALREADY_EXISTS,
  })
  .handler(async ({ context, input }) => {
    try {
      return await updateDeviceGroup({
        ...input,
        requesterId: context.session.user.id,
      });
    } catch (error) {
      return handleError(error) as never;
    }
  });
